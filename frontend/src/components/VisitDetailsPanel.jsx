    ...
    return (
      <div className="mb-4">
        <h5 className="mb-3">
          <i
            className={`bi ${
              type === "prescriptions" ? "bi-capsule" : "bi-card-checklist"
            } me-2`}
          ></i>
          {title}
        </h5>
          ...
          items.map((item) => (
            <div key={item[idKey]} className="p-3 mb-3 border rounded">
              ...
              {type === "referrals" && (
                <>
                  <div>
                    <strong>Exam:</strong> {item.examination_id} –{" "}
                    {item.examination?.name || "N/A"}
                  </div>
                  <div>
              ...
              {type === "diagnoses" && (
                <>
                  <div>
                    <strong>Disease:</strong> {item.disease_id} –{" "}
                    {item.disease?.name || "N/A"}
                  </div>
                  <div>
                    <strong>Date:</strong> {item.diagnosis_date}
                  </div>
                  <div>
      ...
      <div className="card-body p-5 text-center">
        ...
        >
          ...
          {!hidePatientInfo && (
            <>
              <div className="mb-3 border-bottom pb-2">
                <strong>Patient:</strong> {getPatientName(visit.reservation_id)}
              </div>
              <div className="mb-3 border-bottom pb-2">
                <strong>Patient PESEL:</strong>{" "}
                {getPatientPESEL(visit.reservation_id)}
              </div>
            </>
          )}
          <div className="mb-3 border-bottom pb-2">
