...
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, CartesianGrid, Legend, Rectangle } from "recharts";
import { apiRequest } from "../services/apiClient";

const CustomBar = (props) => {
const { x, y, width, height, startHour, fill } = props;
const newX = (startHour / 24) * width;
return <Rectangle x={x + newX} y={y} width={width * (props.value / 24)} height={height} fill={fill} />;
};
...
return user ? `${user.first_name} ${user.last_name}` : `Doctor ${doctor_id}`;
};
...
return (
<div style={{ width: "100%", height: 600 }}> <ResponsiveContainer>
<BarChart
layout="horizontal"
data={chartData}
...
dataKey="date"
width={120}
label={{ value: "Date", angle: -90, position: "insideLeft", offset: 10 }}
/>
<Tooltip
formatter={(value, name, props) => {
const start = props.payload.startHour;
const end = start + props.payload.duration;
const formatHour = h => `${Math.floor(h)}:${Math.round((h%1)*60).toString().padStart(2,'0')}`;
return [`${formatHour(start)} - ${formatHour(end)}`, props.payload.doctor];
}}
/> <Legend />
...
shape={<CustomBar startHour={entry.startHour} fill={entry.color} />}
/>
