  ...
  useEffect(() => {
    const loadPrescription = async () => {
      try {
        ...
        const data =
          user?.role === "patient"
            ? await resourceService.getMyPrescription(id)
            : await resourceService.getPrescription(id);
        setItem(data || null);
      } catch (err) {
        console.error(err);
      ...
      <div className="container py-5 d-flex justify-content-center">
        ...
        >
          ...
          <div className="mb-3 pb-3 border-bottom">
            ...
            <p>
              <span className="fw-bold">Visit ID:</span> {item.visit_id}
            </p>
            <p>
              <span className="fw-bold">Visit Date:</span> {item.visit_date}
            </p>
            {user?.role !== "patient" && (
              <>
                <p>
                  <span className="fw-bold">Patient:</span> {item.patient_name}
                </p>
                <p>
                  <span className="fw-bold">PESEL:</span> {item.patient_pesel}
                </p>
              </>
            )}
            <p>
              <span className="fw-bold">Doctor:</span> {item.doctor_name}
            </p>
          </div>
          ...
          <div className="mb-4">
            <p>
              <span className="fw-bold">Medication:</span> {item.medication}
            </p>
            <p>
              <span className="fw-bold">Dosage:</span> {item.dosage}
            </p>
            <p>
