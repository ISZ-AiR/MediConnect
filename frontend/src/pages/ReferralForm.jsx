...
const ReferralForm = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const existingReferral = await resourceService.getReferralByVisit(visit_id);
        if (existingReferral) {
          setForm({
          ...
          setExists(true);
          setReferralId(existingReferral.referral_id);
        }
  ...
  const handleSubmit = async () => {
    setLoading(true);
    setError(null);
    console.log(visitData.patient_id, visitData.doctor_id);
    try {
      const payload = {
      ...
      if (exists) {
        await resourceService.updateReferral(referralId, payload);
      } else {
    ...
    <div className="min-vh-100 bg-light">
      ...
      <div className="container py-5">
        <div className="card shadow-sm border-0 p-5 text-center">
          ...
          <h2 className="fw-bold mt-3 mb-3">{exists ? "Edit Referral" : "Add Referral"}</h2>
          ...
          <div className="mt-4 d-grid gap-2">
            ...
            <button
                ...
                onClick={() => navigate(`/doctor/visits/${visit_id}`)}
            >
              Cancel
            </button>
          </div>
