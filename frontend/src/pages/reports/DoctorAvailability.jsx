...
const DoctorAvailabilityReport = () => {
  ...
  const fetchReport = async () => {
    ...
    const combinedData = availabilityData.map(doctor => {
      const dailyData = doctor.slots.map(slot => {
        const dayWorkload = workloadMap[doctor.doctor_id]?.find(d => d.date === slot.date) || { reservations: 0, visits: 0 };
  ...
  const handleDoctorCheckbox = (doctor_id) => {
    setSelectedDoctors((prev) =>
      prev.includes(doctor_id) ? prev.filter((id) => id !== doctor_id) : [...prev, doctor_id]
    );
  };
...
const filteredData = selectedDoctors.length === 0
  ? data
  ...
  const exportReportExcel = () => {
    ...
    saveAs(new Blob([wbout], { type: "application/octet-stream" }), "doctor_availability.xlsx");
  };
  ...
  const exportReportPDF = async () => {
    ...
    pdf.setFont("helvetica", "bold");
    pdf.text("MediConnect", pageWidth - margin, margin + 10, { align: "right" });
    ...
    while (remainingHeight > 0) {
      const sliceHeightPt = Math.min(remainingHeight, pageHeight - positionY - margin);
      // create temporary canvas slice
      const sliceCanvas = document.createElement("canvas");
      sliceCanvas.width = canvas.width;
      sliceCanvas.height = Math.round(sliceHeightPt * pxPerPt);
      const ctx = sliceCanvas.getContext("2d");
      ctx.drawImage(canvas, 0, Math.round(sourceY * pxPerPt * (1 / pxPerPt)), sliceCanvas.width, sliceCanvas.height, 0, 0, sliceCanvas.width, sliceCanvas.height);
      const sliceData = sliceCanvas.toDataURL("image/jpeg", 0.8);

      pdf.addImage(sliceData, "JPEG", margin, positionY, availableWidth, sliceHeightPt);
          ...
          <div className="row g-3 align-items-end">
            ...
            <div className="col-md-4" ref={dropdownRef}>
              ...
              <div className="dropdown">
                <button className="btn btn-outline-secondary w-100 text-start" type="button" onClick={() => setDropdownOpen((p) => !p)}>
                  {selectedDoctors.length === 0 || selectedDoctors.length === doctorsList.length ? "All" : `${selectedDoctors.length} selected`}
                </button>
                {dropdownOpen && (
                  <div className="dropdown-menu show p-3" style={{ maxHeight: 240, overflowY: "auto" }}>
                    <div className="form-check mb-2">
                      <input className="form-check-input" type="checkbox" id="select-all" checked={selectedDoctors.length === doctorsList.length} onChange={toggleAllDoctors} />
                      <label className="form-check-label" htmlFor="select-all">All</label>
                    </div>
                    {doctorsList.length === 0 ? (
                      ...
                      doctorsList.map((doc) => (
                        <div key={doc.doctor_id} className="form-check">
                          <input className="form-check-input" type="checkbox" id={`doc-${doc.doctor_id}`} checked={selectedDoctors.includes(doc.doctor_id)} onChange={() => handleDoctorCheckbox(doc.doctor_id)} />
                          <label className="form-check-label" htmlFor={`doc-${doc.doctor_id}`}>{doc.doctor_id} - {doc.first_name} {doc.last_name}</label>
                        </div>
                      ))
                    )}
                  </div>
            ...
            <div className="col-md-4 d-flex gap-2">
              ...
              <div className="dropdown">
                <button className="btn btn-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">Export</button>
                <ul className="dropdown-menu">
                  <li><button className="dropdown-item" onClick={exportReportPDF}>PDF</button></li>
                  <li><button className="dropdown-item" onClick={exportReportExcel}>Excel</button></li>
                </ul>
              </div>
            </div>
          </div>
