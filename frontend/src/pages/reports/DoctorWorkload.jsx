...
const DoctorWorkload = () => {
  ...
  const fetchReport = async () => {
    ...
    setLoading(true);
    try {
      const reportData = await statsService.getDoctorWorkload(
        startDate,
        endDate
      );
      setData(reportData);
  ...
  const sortedData = [...data]
    .filter(
      (d) =>
        selectedDoctors.length === 0 || selectedDoctors.includes(d.doctor_id)
    )
    .sort((a, b) => {
                        ...
                        {doctorsList.map((doc) => (
                          <div key={doc.doctor_id} className="form-check">
                            <input
                              ...
                              onChange={() =>
                                handleDoctorCheckbox(doc.doctor_id)
                              }
                            />
                            <label
                              className="form-check-label"
