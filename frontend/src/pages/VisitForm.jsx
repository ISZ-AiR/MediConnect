...
// Fetch pielęgniarek + użytkowników tylko raz
...
// Stabilne funkcje dla useEditableResource
const stableLoadFn = useCallback(resourceService.getVisit, []);
const stableCreateFn = useCallback(resourceService.createVisit, []);
const stableUpdateFn = useCallback(resourceService.updateVisit, []);
...
<i className="bi bi-calendar-plus text-warning" style={{ fontSize: "3rem" }}></i> <h2 className="fw-bold mt-3">{id ? "Edit Visit" : "Create Visit"}</h2> <p className="text-muted">Fill in visit details</p> </div>
            ...
            <form onSubmit={submit}>
              ...
              <FormField
                type="textarea"
                name="visit_note"
                label={<><i className="bi bi-journal-text me-2"></i>Notes</>}
